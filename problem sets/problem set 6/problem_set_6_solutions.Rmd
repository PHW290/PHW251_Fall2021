...
Create two vectors of the pain ratings. One vector holds all of the pre-test while the other vector holds the post-test.

Once you have these vectors create a third vector called "diff" that calculates the difference between post and pre. 

```{r}
# your code here
pre <- placebo_df$PAIN_RATE[which(placebo_df$TEST == "Pre")]
post <- placebo_df$PAIN_RATE[which(placebo_df$TEST == "Post")]
diff <- post - pre


# Uncomment the line below after you create the diff. It will create a table of counts
#table(diff)
```
